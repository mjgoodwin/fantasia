...
class LeagueOperationTest < MiniTest::Spec
  ...
  describe "League::Create" do
    it "persists valid" do
      league = League::Create.(league: { name: "Mickey Mouse League", commissioner: user, rounds: [{ start_time: Time.zone.now.end_of_day }] }).model
    ...
    it "invalid - no name" do
      res, op = League::Create.run(league: { commissioner: user, rounds: [{ start_time: Time.zone.now.end_of_day }] })
    ...
    it "invalid - no commissioner" do
      res, op = League::Create.run(league: { name: "Mickey Mouse League", rounds: [{ start_time: Time.zone.now.end_of_day }] })
  ...
  describe "League::Create" do
    let(:league) { League::Create.(league: { name: "Mickey Mouse League", commissioner: user, rounds: [{ start_time: Time.zone.now.end_of_day }] }).model }
